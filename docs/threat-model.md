...
instead of sending under a fabricated parent (TRN-006). The `cg-foreman`
...
| Undocumented ChatGPT transport (`pi-gpt`) | accepted by the user; vendored and patched in-repo (TRN-006); sends bound to the leaf and reconciled by readback (TRN-004); provider drift caught by the opt-in live lane |
