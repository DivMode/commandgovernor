...
> Transplanted unchanged from frozen PR #16 (upstream-Pi foundation) into the
> Prime-native foundation (Issue #17). "Pi" below reads as "the Pi-family
> substrate"; the substrate is now Prime Agent v0.8.1 (ADR 0009). The
> incarnation and single-authority material it points at is what
> `governor/session/registry.ts` and `governor/mutation/ledger.ts` port.
